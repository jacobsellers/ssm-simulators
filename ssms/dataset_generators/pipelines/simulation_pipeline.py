...
class SimulationPipeline:
    ...
    This pipeline implements the traditional workflow:
    ...
    This pipeline is necessary when analytical solutions are not available
    ...
    def __init__(
        ...
        model_config: dict,
        estimator_builder: EstimatorBuilderProtocol | type,
        training_strategy: TrainingDataStrategyProtocol | type,
    ):
        """Initialize simulation-based pipeline.

        Args:
            generator_config: Configuration for data generation
            model_config: Model specification (must contain 'param_bounds_dict')
            estimator_builder: Builder for KDE estimator (class or instance)
                - If a class is passed, it will be instantiated with generator_config
                - If an instance is passed, it will be used as-is
            training_strategy: Strategy for generating training samples (class or instance)
                - If a class is passed, it will be instantiated with generator_config and model_config
                - If an instance is passed, it will be used as-is
        """
        ...
        if isinstance(training_strategy, type):
            # It's a class - instantiate with both configs
            self.training_strategy = training_strategy(generator_config, model_config)
        else:
            # It's an instance - use as-is
    ...
    def _run_simulations(self, theta: dict, seed: int | None) -> dict:
        ...
        simulations = self.simulator.simulate(
            ...
            n_samples=self.generator_config["simulator"]["n_samples"],
            delta_t=self.generator_config["simulator"]["delta_t"],
            max_t=self.generator_config["simulator"]["max_t"],
            random_state=seed,
        )
    ...
    def _is_valid_simulation(self, simulations: dict) -> tuple[bool, np.ndarray]:
        ...
        for choice_tmp in simulations["metadata"]["possible_choices"]:
            ...
            keep = (
                ...
                & (mean_ <= filters.get("mean_rt", float("inf")))
                & (std_ >= filters.get("std", 0.0))
                & (mode_cnt_rel_ <= filters.get("mode_cnt_rel", 1.0))
                & (tmp_n_c >= filters.get("choice_cnt", 0))
            )
